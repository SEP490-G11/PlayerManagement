from odoo import _
from odoo.addons.web.controllers.export import ExportFormat
from odoo.addons.z_web.helpers.constants import EXPORT_DATE_FORMAT
from odoo.addons.z_web.helpers.utils import ZUtils


_super_filename = ExportFormat.filename


def filename(self, base):
    res = _super_filename(self, base)
    title_combo = _("List-combo")
    title_combo_order = _("List-combo-order")
    if base == "z_combo.combo":
        return f"{title_combo}-{ZUtils.format_datetime(ZUtils.now(), EXPORT_DATE_FORMAT)}"
    if base == "z_combo.combo.order":
        return f"{title_combo_order}-{ZUtils.format_datetime(ZUtils.now(), EXPORT_DATE_FORMAT)}"
    return res


ExportFormat.filename = filename
