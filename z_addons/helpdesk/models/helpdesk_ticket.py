...
class HelpdeskTicket(models.Model):
    ...
    date_deadline_recent =  fields.Date(string="Date deadline recent", compute="_compute_date_deadline_recent")
    ...
    def _compute_date_deadline_recent(self):
        """ Compute the deadline for the ticket, based on the current date and the SLA of the ticket """
        for ticket in self:
            deadlines = ticket.activity_ids.mapped('date_deadline')
            future_deadlines = [day for day in deadlines if day and day >= fields.Date.today()]
            ticket.date_deadline_recent = min(future_deadlines) if future_deadlines else False